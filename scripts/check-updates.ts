#!/usr/bin/env tsx
/**
 * Chadian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawchadian.com';
const PORTAL_NAME = 'Chad Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/chadian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Chadian Law MCP — Data Freshness Check');
