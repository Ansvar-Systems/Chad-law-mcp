#!/usr/bin/env tsx
/**
 * Chad Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawchadian.com. Chadian proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawchadian.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'chadian-law-mcp/1.0 (https://github.com/Ansvar-Systems/chadian-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Chad Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawchadian.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawchadian.com (Federal Negarit Gazette PDFs)`);
