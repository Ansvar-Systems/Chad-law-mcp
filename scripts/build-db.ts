#!/usr/bin/env tsx
/**
 * Database builder for Chad Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Chad Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Chadian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'lawchadian.com');
