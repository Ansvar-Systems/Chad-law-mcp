/**
 * get_chadian_implementations — Find Chadian statutes implementing a specific EU directive/regulation
...
export interface GetChadianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface ChadianImplementationResult {
  document_id: string;
...
export async function getChadianImplementations(
  db: InstanceType<typeof Database>,
  input: GetChadianImplementationsInput,
): Promise<ToolResponse<ChadianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as ChadianImplementationResult[];
