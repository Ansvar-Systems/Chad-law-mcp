/**
 * search_eu_implementations — Search EU directives/regulations that have Chadian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_chadian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as chadian_statute_count
  ...
  if (input.has_chadian_implementation) {
    sql += ' HAVING chadian_statute_count > 0';
  }
